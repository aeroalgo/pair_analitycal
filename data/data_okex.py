...
def get_history(start_date: str, end_date: str, timeframe: str):
    ...
    response = requests.get('https://www.okex.com/api/v5/market/tickers?instType=SWAP')
    ...
    for val, data in enumerate(get_market_data['data'], 0):
        symbol_okex = re.sub('[^0-9a-zA-Z]+', '', data["instId"])
        if symbol_okex[-7:] != 'USDSWAP':
            if symbol_okex[:-4] in symbol_spot_binance:
