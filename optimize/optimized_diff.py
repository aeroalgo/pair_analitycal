...
import time
from optimize.get_optimize_diff import get_moving, trade_spread
import os.path
...
def optimize(data_name_pair: list):
    ...
    volatility_spread = np.around(arange(0.7, 1.4, 0.1), 1)
    for namepair in data_name_pair:
        ...
        if True:
            ...
            sma_src1 = talib.SMA(source1, 2000)
            sma_src1 = sma_src1 / source2
            sma_src1 = talib.SMA(sma_src1, 2000)
            lot_src1 = money / 2 / source1
            lot_src2 = lot_src1 * sma_src1
            ...
            if deviation_level == 'all':
                for deviation in levels_param:
                    levels = source1 * lot_src1 * deviation
                    levels_dict[deviation] = levels
            elif deviation_level != 'all':
                levels = source1 * lot_src1 * float(deviation_level)
                levels_dict[deviation_level] = levels
            for vol in volatility_spread:
                spread = source1 * lot_src1 - source2 * lot_src2 * vol
                ma_dict = get_moving(ma_param, spread, ma_num, ma_period_opt)
                volatility_ma[vol] = ma_dict
            levels_ma = [levels_dict, volatility_ma]
            start_time = time.time()
            eq = []
            buy1 = trade_spread(source2, source1, levels_ma, 1, money, eq, 1, lot_src2, lot_src1)
            if not math.isnan(buy1[0][0]['Profit']):
                sell1 = trade_spread(source1, source2, levels_ma, -1, money, buy1[1], 1, lot_src1, lot_src2)
                if not math.isnan(sell1[0][0]['Profit']):
                    buy2 = trade_spread(source2, source1, levels_ma, 1, money, sell1[1], 2, lot_src2, lot_src1)
                    sell2 = trade_spread(source1, source2, levels_ma, -1, money, buy2[1], 2, lot_src1, lot_src2)
                    print(buy1[0])
                    print(buy2[0])
                    print(sell1[0])
                    print(sell2[0])
...
if __name__ == '__main__':
    ...
    client_mongo = MongoClient('localhost', 27017)
    db = client_mongo['OptimizedData']
    collection = 'diff_rough_opt_' + config.ma_type[ma_num]
