...
import json
import os.path
from optimize.get_optimize_diff import trade_spread_rough
import pandas as pd
import time
import numpy as np
...
def opt(source: list):
    ...
    for name in source:
        ...
        spread = source1 * lot_src1 - source2 * lot_src2
        ma_type = {
        ...
        kat = source1 * lot_src1 * 0.015
        indicator_spread = spread - sma_spread
        source1 = source1.tolist()
        source2 = source2.tolist()
        ind = indicator_spread.tolist()
        kat = kat.tolist()
        buy1 = trade_spread_rough(source2, source1, ind, kat, 1, 1, money, lot_src2, lot_src1)
        sell1 = trade_spread_rough(source1, source2, ind, kat, 1, -1, money, lot_src1, lot_src2)
        s1.append(name[0])
        s2.append(name[1])
        p.append(buy1 + sell1)
        opt_list['Source1'] = s1
...
def main():
    ...
    name_collection = 'diff_rough_opt_' + config.ma_type[ma_num]
